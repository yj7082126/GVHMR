...
from hmr4d.network.dino import load_dinov3
from hmr4d.network.hmr2.utils.preproc import IMAGE_MEAN, IMAGE_STD
from hmr4d.utils.camera.camera_embed import CameraEncoder
...
class Pipeline(nn.Module):
    def __init__(self, args, args_denoiser3d, **kwargs):
        ...
        # Optional online DinoV3 image encoder.
        # If enabled and image+bbx_xys are available in batch, we use DinoV3 features as f_imgseq.
        self.use_dinov3_online = bool(getattr(args, "use_dinov3_online", False))
        self.dinov3_model_type = str(getattr(args, "dinov3_model_type", "dinov3_vith16plus"))
        self.dinov3_crop_size = int(getattr(args, "dinov3_crop_size", 512))
        # Keep online image encoder inference-only to avoid DDP unused-parameter issues.
        self.dinov3_freeze = bool(getattr(args, "dinov3_freeze", True))
        self.dinov3_use_amp = bool(getattr(args, "dinov3_use_amp", False))
        self.dinov3_pool = str(getattr(args, "dinov3_pool", "mean"))  # mean | none
        self.dinov3_use_precropped_image = bool(getattr(args, "dinov3_use_precropped_image", True))
        self.use_camera_encoder = bool(getattr(args, "use_camera_encoder", True))

        self.dinov3 = None
        if self.use_dinov3_online:
            self.dinov3 = load_dinov3(model_type=self.dinov3_model_type)
            self.dinov3.eval()
            for p in self.dinov3.parameters():
                p.requires_grad_(False)
            self.register_buffer("dino_image_mean", IMAGE_MEAN.view(1, 3, 1, 1), persistent=False)
            self.register_buffer("dino_image_std", IMAGE_STD.view(1, 3, 1, 1), persistent=False)
            if self.use_camera_encoder:
                self.camera_encoder = CameraEncoder(embed_dim=self.dinov3.embed_dim, patch_size=self.dinov3.patch_size)
                # self.camera_encoder.eval()
                # for p in self.camera_encoder.parameters():
                #     p.requires_grad_(False)

    def _compute_crop_intrinsics(self, K_fullimg, bbx_xys, crop_size):
        """
    ...
    def _compute_dinov3_embedding(self, inputs):
        """Compute DinoV3 features from `image` and `bbx_xys`.
        Returns:
            (feat, used_camera_encoder)
            feat is None or tensor of shape (B, L, C) when dinov3_pool='mean',
            or (B, L, C, H, W) when dinov3_pool='none'.
        """
        ...
        if "image" not in inputs or inputs["image"] is None:
            return None, False
        self.dinov3.eval()
        used_camera_encoder = False

        images = inputs["image"]
        if images.dim() != 5:
            return None, False

        # Accept both (B, L, H, W, 3) and (B, L, 3, H, W)
        if images.shape[-1] == 3:
        ...
        if self.dinov3_use_precropped_image:
            ...
            rois = torch.stack(
                [
                    torch.arange(B * L, device=images.device, dtype=images.dtype),
                    x1,
                    y1,
                    x2,
                    y2,
            ...
            crops = roi_align(
                images,
                rois.float(),
                output_size=(self.dinov3_crop_size, self.dinov3_crop_size),
                spatial_scale=1.0,
                aligned=True,
            )

        # Normalize to ImageNet convention used by Dino/HMR2 preprocessing.
        if crops.max() > 1.5:
            crops = crops / 255.0
        crops = (crops - self.dino_image_mean) / self.dino_image_std

        # DinoV3 is encoding-only: keep no-grad for speed/memory.
        with torch.no_grad():
            with autocast(enabled=self.dinov3_use_amp):
                dino_feat = self.dinov3({"img": crops})  # (B*L, C, H', W')
        ...
        if self.use_camera_encoder and hasattr(self, "camera_encoder"):
            if ("K_fullimg" in inputs and inputs["K_fullimg"] is not None
                ...
                used_camera_encoder = True

        if self.dinov3_pool == "mean":
            dino_feat = dino_feat.mean(dim=(-2, -1))  # (B*L, C)
            dino_feat = dino_feat.view(B, L, -1)
        else:
            dino_feat = dino_feat.view(B, L, *dino_feat.shape[1:])
        return dino_feat, used_camera_encoder

    def _camera_encoder_ddp_keepalive(self):
        """Create a zero-valued term that touches camera_encoder params for DDP graph parity."""
        if not (self.use_camera_encoder and hasattr(self, "camera_encoder")):
            return None
        if not any(p.requires_grad for p in self.camera_encoder.parameters()):
            return None
        z = None
        for p in self.camera_encoder.parameters():
            term = p.sum() * 0.0
    ...
    def forward(self, inputs, train=False, postproc=False, static_cam=False):
        ...
        f_dino_imgseq, used_camera_encoder = self._compute_dinov3_embedding(inputs)
        ...
        # Keep DDP reducer happy on batches without image input while camera_encoder is trainable.
        if train and not used_camera_encoder:
            keepalive = self._camera_encoder_ddp_keepalive()
            if keepalive is not None:
                total_loss = total_loss + keepalive
